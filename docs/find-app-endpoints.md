...
## 그래도 내 것만 보고 싶다면 — 로컬 전용 2단계

본인 컴퓨터에서만 돌아갑니다. CI 에서는 막혀 있습니다.

```bash
# 1. 브라우저가 열리면 직접 로그인하고 혜택 목록까지 간 뒤 터미널에서 Enter
npm run collect -- --login hana-pick

# 2. 저장된 세션으로 수집
npm run collect -- --auth --only hana-pick
```
...
`data/feed.local.js` 가 있으면 페이지가 그것도 함께 읽어 **"내 계정"** 배지를 달고 순위에
넣습니다. 이 파일은 내 컴퓨터에만 있으므로, 배포된 페이지나 다른 사람 화면에는 나오지 않습니다.

세션이 만료되면 1단계를 다시 하면 됩니다. 카드사 세션은 대개 짧게는 몇십 분입니다 —
로그인 수집이 "자동 갱신"이 되기 어려운 또 하나의 이유입니다.
